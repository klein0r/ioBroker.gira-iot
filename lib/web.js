...
function GiraWebExtension(server, webSettings, adapter, instanceSettings, app) {
    ...
    (function __constructor() {
        if (!that.settings.secure) {
        ...
        } else if (that.settings.bind === '0.0.0.0') {
            ...
            that.adapter.sendTo(that.namespace, 'registerCallbacks', { baseUrl: `https://${that.settings.bind}:${that.settings.port}/${that.namespace}` });
