...
function GiraWebExtension(server, webSettings, adapter, instanceSettings, app) {
    ...
    this.registerCallbackUrl = () => {
        const callbackBaseUrl = `https://${this.adapter.config.bind}:${this.settings.port}/${this.namespace}`;
        this.adapter.log.info(`Setting (new) callback url to ${callbackBaseUrl}`);
        this.adapter.sendTo(that.namespace, 'registerCallbacks', { baseUrl: callbackBaseUrl });
    };

    this.stateChange = (id, state) => {
        if (`${that.namespace}.info.connection` === id && state && state.ack) {
            if (state.val) {
    ...
    (function __constructor() {
        if (!that.settings.secure) {
            ...
            that.adapter.log.error('Unable to register web extension - please select other address than 0.0.0.0');
        } else {
